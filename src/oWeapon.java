...
final class oWeapon extends oGameObject {
	...
	final void I(sprite_group sprite_group1, sprite_group sprite_group2, int i, int j, int k, boolean flag, int l,
		...
		random = j;
		LI = k;
		abs = 40 + (int) (Math.random() * (double) LI) + random;
		MI = flag;
		PI = l;
		QI = i1;
		UI = f;
	...
	final void I() {
		if (super.x != null)
			if (F) {
				...
				super.J = super.g.J + (float) Math.round(I * f3 - Z * f4);
				super.S = super.g.S + (float) Math.round(I * f4 + Z * f3);
				F();
			} else {
				super.y = super.x.I(super.d, super.b);
				super.J = super.g.J + I;
			...
			if (OI == 2) {
				...
				if (abs <= 0) {
					if (((oSpaceship) super.g).D > 0)
						abs();
					abs = (int) (Math.random() * (double) LI) + random;
					return;
				}
			} else {
				abs--;
		...
		if (MI) {
			...
			for (int k = 0; k <= super.owner.eZ; k++) {
				...
				if (oWeaponfire2 != null) {
					...
					oWeaponfire2.I(((oGameObject) (oWeaponfire2)).J + f1 * (float) j * (float) k,
							((oGameObject) (oWeaponfire2)).S + f2 * (float) j * (float) k, f3, f4, true);
					if (XI != -1)
	...
	final void abs() {
		...
		if (oSpaceship1.WI != null) {
			...
			float f = (((oGameObject) (oSpaceship1)).J + (float) ((oGameObject) (oSpaceship1)).y.E + I * f4) - Z * f5;
			float f1 = ((oGameObject) (oSpaceship1)).S + (float) ((oGameObject) (oSpaceship1)).y.G + I * f5 + Z * f4;
