...
final class sprite_group {
	...
	final int I(float f) {
		int i;
		if (J)
			i = Math.round(((f - 3.141593F / (float) (Z * 2)) / 3.141593F) * (float) Z);
		else
	...
	final void I(float f, int i, int j, int k, int l, int i1, int j1, int k1, imagepixels imagepixels, int l1, int i2,
		...
		for (int k2 = i2; k2 < j2; k2++) {
			sprite sprite1 = sprite.I(f, (float) ((6.2831853071795862D / (double) l1) * (double) k2), i, j, k,
					l, i1, j1, k1, imagepixels, surface, HNSM);
			if (sprite1 != null)
				I(k2, sprite1);
		}
	...
	final void I(int i, float f, int j, int k, int l, int i1, int j1, int k1, int l1, imagepixels imagepixels, int i2,
		...
		for (int l2 = j2; l2 < k2; l2++) {
			sprite sprite1 = sprite.I(f, (float) ((6.2831853071795862D / (double) i2) * (double) l2), j, k, l,
					i1, j1, k1, l1, imagepixels, surface, HNSM);
			if (sprite1 != null)
				I(i, sprite1);
		}
