...
class oGameObject {
	...
	void I(surface surface1) {
		if (y != null && w) {
			if (t > 0) {
				if (n <= 0)
					surface1.Z((int) J + ((gamebase) (owner)).WC, (int) S + ((gamebase) (owner)).XC, (y.D + 10) - sqrt * 3,
							(y.F + 10) - sqrt * 3, ((gamebase) (owner)).MB.I(sqrt, t));
				else
	...
	final void I(oGameObject oGameObject1) {
		...
		float f2 = i - Y;
		if ((double) f2 < -3.1415926535897931D)
			f2 += 6.283185F;
		else if ((double) f2 > 3.1415926535897931D)
			f2 -= 6.283185F;
		if (f2 < -z)
			Y -= z;
		else if (f2 > z)
	...
	boolean Z(oGameObject oGameObject1) {
		if (A < oGameObject1.G && G > oGameObject1.A && E < oGameObject1.H && H > oGameObject1.E && atan(oGameObject1)) {
