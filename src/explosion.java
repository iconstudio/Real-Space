...
final class explosion extends oGameObject
{
    ...
    final void I(sprite_group sprite_group1, int i, float f, float f1, float f2, float f3, int j, 
            int k, int l, boolean flag)
    {
        ...
        {
            Z = 10 + l / 5;
            D = Math.min(D + 15 + (int)((double)l * 0.20000000000000001D), 40);
            if(super.owner.QC == 2)
                D = D / 2;
            else
            if(super.owner.QC == 3)
            ...
            {
                min[i1] = (float)((Math.random() - 0.5D) * ((double)l * 0.050000000000000003D + 8D));
                random[i1] = (float)((Math.random() - 0.5D) * ((double)l * 0.050000000000000003D + 8D));
                lightGray[i1] = f + min[i1] * 5F;
                max[i1] = f1 + random[i1] * 5F;
            }

            C = (float)((double)l * 0.20000000000000001D + 10D);
            B = C;
            if(flag)
            {
                int j1 = super.m / 2;
    ...
    final void I(surface surface1)
    {
        if(super.w)
        {
            ...
            if(super.s < Z && D > 0)
            {
                ...
                for(int j = 0; j < D; j++)
                    surface1.I((int)lightGray[j] + ((gamebase) (super.owner)).WC, (int)max[j] + ((gamebase) (super.owner)).XC, 3, 3, i);
